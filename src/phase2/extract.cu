...
// Hàm thực hiện trích xuất và GHI TRỰC TIẾP vào file nhị phân
// Tránh lưu trữ vector khổng lồ trong RAM giúp tiết kiệm bộ nhớ
                               ...
                               const std::string& label_file) 
{
    ...
    // 1. Cấu hình kích thước đặc trưng (128 * 8 * 8 = 8192)
    const int latent_dim = 128 * 8 * 8;
    size_t input_bytes = 3 * 32 * 32 * sizeof(float);
    
    // 2. Tạo một buffer trên Host để nhận dữ liệu từ hàm extract_features
    std::vector<float> h_feat_buffer(latent_dim);

    auto start = std::chrono::high_resolution_clock::now();

    for (size_t i = 0; i < images.size(); ++i) {
        // Bước A: Copy ảnh lên GPU đầu vào của Autoencoder
        cudaMemcpy(ae.d_input, images[i].data(), input_bytes, cudaMemcpyHostToDevice);
        
        // Bước B: Gọi hàm của bạn (Đúng 2 tham số: d_in và h_out)
        // Dữ liệu sẽ được copy từ GPU vào h_feat_buffer.data() bên trong hàm này
        ae.extract_features(ae.d_input, h_feat_buffer.data());
        
        // Bước C: Ghi buffer Host vào file nhị phân
...
int main() {
    ...
    // 3. Thực hiện trích xuất tập TRAIN
    ...
    // 4. Thực hiện trích xuất tập TEST
